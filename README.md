...
- Team Mode tools: `team_create`, `team_list`, `team_send_message`, `team_inbox`, `team_task_create`, `team_task_update`, `team_task_start`, `team_coordinator_run`, `team_round_run`, and `team_status` provide a persistent shared message bus, communication rounds, and task board for lead/member agent coordination
...
Team Mode is a persistent coordination layer rather than a terminal UI. It stores teams under `~/.codex-claude/teams/*.json`; a lead agent can create a team, create shared tasks, send messages to one member or all members, and members can read their inbox. The dashboard exposes Team Mode in Advanced view so users can create teams, inspect members, send messages, add shared tasks, run role-aware communication rounds, and start a real delegated agent from a team task. `team_round_run` records one collaboration round into the shared timeline, with each participant posting a role-aware message about scope, execution, review, or merge risks. By default those messages are deterministic role-aware summaries; with `liveAgents: true`, each participant's configured provider is called in a read-only sync stage and the provider output is saved into the round. With `createTasks: true`, round messages become follow-up Team Mode tasks assigned back to the speaker. `team_task_start` uses the assignee member's agent/profile by default, launches a delegated task, marks the team task `in_progress`, writes `linkedTaskId`, and records a team message. Team status reads automatically sync linked delegated task outcomes back to the team board: completed becomes `done`, failed or interrupted becomes `blocked`, cancelled becomes `cancelled`, and active tasks stay `in_progress`. The dashboard also shows the linked task status, summary, coordinator state, and round timeline inline. Use `team_task_update.linkedTaskId` when linking an existing delegated task manually.
...
- The bridge default Claude model follows your local Claude Code settings (`~/.claude/settings.json`, for example `opus[1m]`). You can override it with `defaults.claudeModel`, `CODEX_CLAUDE_MODEL`, Dashboard Advanced mode, or the MCP `model` argument.
- Before launching Claude Code, the bridge refreshes Claude-related environment variables from the user's shell (`ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_API_KEY`, and `CLAUDE_CODE_*`) so Codex Desktop does not keep using a stale proxy endpoint after `.zshrc` changes.
